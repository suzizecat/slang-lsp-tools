...
public:
  ...
  bool TestProduct(const json &p) { return client.CallMethod<bool>(1, "TestProduct", {p}); }
...
void doWarehouseStuff(IClientConnector &clientConnector) {
  ...
  test["id"] = 18;
  test["price"] = 1.2;
  test["name"] = "TestProduct";
  appClient.TestProduct(test);
...
int main() {
  ...
  rpcServer.Add("TestProduct", GetHandle(&WarehouseServer::TestProduct, app), {});
