...
namespace diplomat::app
{
    class LSPDiagnosticClient : public slang::DiagnosticClient
    {
         protected:
            ...
            std::unordered_map<std::string, std::unique_ptr<lsp::types::PublishDiagnosticsParams> > _diagnostics;
            lsp::types::PublishDiagnosticsParams* _last_publication;
            const slang::SourceManager* _sm;

            void _remap_internal_diagnostic_uri(lsp::types::PublishDiagnosticsParams* diag, const std::string& old_uri, const std::string& new_uri);
        public:
            ...
            inline const std::unordered_map<std::string, std::unique_ptr<lsp::types::PublishDiagnosticsParams> >& get_publish_requests() const {return _diagnostics; };
    };
